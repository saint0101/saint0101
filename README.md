




# N'GUESSAN Konan Saint-Fourier Onésyme

### 🛠️ Ingénieur Systèmes & Génie Logiciel · DevOps
**ARTCI – Autorité de Régulation des Télécommunications/TIC de Côte d'Ivoire 🇨🇮**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=flat&logo=django&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat&logo=ansible&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)




---

## À propos de moi

Ingénieur systèmes, génie logiciel et DevOps avec **plus de 7 ans d'expérience**.
Je conçois et déploie des architectures applicatives robustes — du monolithe aux microservices, du bare metal au cloud.

- 🏢 **ARTCI** — Autorité de Régulation des Télécommunications/TIC de Côte d'Ivoire
- 📍 Abidjan, Côte d'Ivoire
- 📧 fouriersaint@gmail.com
- 💼 [LinkedIn](https://www.linkedin.com/in/konan-n%E2%80%99guessan-b78b15a4/)
-  Formations: ISO 27001/27002/27005 · LPIC-1 · Ethical Hacking

---

## Projets phares

| Projet | Description | Stack |
|--------|-------------|-------|
| 🛡️ [consommateurs.artci.ci](https://consommateurs.artci.ci) | Plateforme nationale de gestion des plaintes usagers (téléphonie, Mobile Money, données personnelles) | Django · PostgreSQL · Docker · CI/CD |
| 📅 [meeting.artci.ci](https://meeting.artci.ci) | Gestion intelligente des réunions avec QR codes pour présences | Python · REST API · Kubernetes |
| 🏘️ [ivoirimmo.ci](https://ivoirimmo.ci) | Plateforme immobilière de référence en Côte d'Ivoire | Full-stack · Cloud · UX/UI |
| 🔒 Anonymax | Système d'anonymisation des données (conformité RGPD) | Python · ISO 27701 |

---

## Stack technique

```text
Langages     Python · JavaScript · Perl · Bash · Dart/Flutter
Frameworks   Django · REST APIs · Bootstrap
DevOps       Docker · Kubernetes · Ansible · CI/CD · Git
Bases BDD    PostgreSQL · MySQL
Supervision  Centreon · Nagios
Sécurité     ISO 27001/27002/27005 · Ethical Hacking · LPIC-1
Cloud        Virtualisation · Architecture 2-tiers / 3-tiers · Microservices
```

---

## Formation & Certifications

- **Master IT** – Développement Logiciel · IBM-T Abidjan (2020)
- **Licence SIGL** – Systèmes Informatiques & Génie Logiciel · ESATIC (2016)
- LPIC-1 Linux Administrator · Docker DevOps (Udemy) · Ethical Hacking (Udemy)
- Flutter & Dart · Python Advanced · Smart Africa Digital Academy (2025)

---

## GitHub Stats

<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=saint0101&show_icons=true&theme=github_dark&hide_border=true" height="150"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=saint0101&layout=compact&theme=github_dark&hide_border=true" height="150"/>
</div>

---

<div align="center">
📫 fouriersaint@gmail.com  ·  🌍 Abidjan, Côte d'Ivoire  ·  💼 LinkedIn
</div>
